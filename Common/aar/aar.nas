...
var update_loop = func {
	...
	var tankers = [];
	var activeTanker=-1;
	var activeDistance=getprop("systems/refuel/range");
	if (ai_enabled) {
		...
		for (var i=0; i<ac; i=i+1) {
			...
			if (getprop(test)) {
				if (getprop("ai/models/multiplayer["~i~"]/distance-to-nm")*1825<activeDistance) {
					append(tankers, i);				
					activeDistance=getprop("ai/models/multiplayer["~i~"]/distance-to-nm")*1825;
					activeTanker=i;
				}
			}
		}
