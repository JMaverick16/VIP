...
var CDU = {
	...
	Field : {
		...
		isSelectable: func { me._selectable; },
			...
			copyData: func(lsk) {
				...
				if ((CDU.isRightLSK(lsk) != me.alignRight) or (index < 0) or (index >= me.lineCount())) return nil;
            
				var d = me.data(index);
				if ((d == nil) or (size(d) == 0)) return nil;
            
				# if we contain placeholder, dont return that             
				if ((d[0] == `#`) or (substr(d, 0, 2) == '--')) return nil;
			...
			enterData: func(lsk, scratch) {
				var index = me._page.indexForLine(me, CDU.lineForLSK(lsk));
				if ((CDU.isRightLSK(lsk) != me.alignRight) or (index < 0) or (index >= me.lineCount())) return -1;
	...
	AbstractModel : {
		...
		lineCountFor: func(tag) { 
			me._callTagMethod('countFor' ~ tag, [], 0); },
	...
	Page : {
		...
		title: func {
			# no siblings, simple
			if ((me._previousPage == nil) and (me._nextPage == nil)) return me.baseTitle;
                
			var pgIndex = 0;
			var pgCount = 0;
			var pg = me;
			# find the group leader
			while (pg._previousPage != nil) pg = pg._previousPage;
			...
			while(size(pgTitle) < CDU.NUM_COLS-size(pgText)) pgTitle ~= " ";
		...
		# no-op by default, called when the page is replaced / cleared
			...
			_updateActions: func(cdu) {
				...
				if (leftAct > 0 and leftAct < 99) cdu.setRowText(cdu.rowForLineTitle(leftAct), 0, 0, '------------');
        
				if (rightAct > 0 and rightAct < 99) cdu.setRowText(cdu.rowForLineTitle(rightAct), CDU.NUM_COLS - 1, 1, '------------');
			},
		...
		MultiPage : {
			...
			title: func { 
				...
				var pgTitle = me.baseTitle;
				while(size(pgTitle) < CDU.NUM_COLS-size(pgText)-1) pgTitle ~= " ";
				return pgTitle~pgText;
			},
			...
			visibleIndices: func(field) {
				...
				return { firstIndex:fieldStart - fieldBase, lastIndex:fieldEnd - fieldBase, firstLine: firstLine};
			},
	...
	setRowText: func(row, col, alignRight, text) {
		...
		foreach(var textItem; text1) {
			...
			textL ~= text2[0];
			while (size(textS) < size(textL)) textS ~= ' ';
			if (size(text2) > 1) textS ~= text2[1];
			while (size(textL) < size(textS)) textL ~= ' ';
		}
	...
	cleanup: func {
		for (r=0; r<CDU.NUM_ROWS; r=r+1) me.clearRowText(r);
	},
